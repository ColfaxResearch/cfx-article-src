...
from cutlass.utils import dynamic_persistent_tile_scheduler as _clc_sched

_ClcParams = _clc_sched.ClcDynamicPersistentTileSchedulerParams
_ClcScheduler = _clc_sched.ClcDynamicPersistentTileScheduler
_orig_get_grid_shape = _ClcParams.get_grid_shape
_orig_swizzle_and_rasterize = _ClcScheduler._swizzle_and_rasterize


@_dsl_user_op
def _patched_params_get_grid_shape(self, *, loc=None, ip=None):
    x, y, z = _orig_get_grid_shape(self, loc=loc, ip=ip)
    if self.swizzle_size == 1 and self._raster_along_m:
        return (x, y, z)  # natural (m, n, l) grid: nothing to fix
    return (z, y, x)  # linear cluster count: gridDim.z (cap 65535) -> gridDim.x


@_cute.jit
def _patched_swizzle_and_rasterize(self, x_idx, y_idx, z_idx, *, loc=None, ip=None):
    if _cutlass.const_expr(
        self.params.swizzle_size == 1 and self.params._raster_along_m
    ):
        return _orig_swizzle_and_rasterize(self, x_idx, y_idx, z_idx, loc=loc, ip=ip)
    return _orig_swizzle_and_rasterize(self, z_idx, y_idx, x_idx, loc=loc, ip=ip)


_ClcParams.get_grid_shape = _patched_params_get_grid_shape
_ClcScheduler._swizzle_and_rasterize = _patched_swizzle_and_rasterize
...
def get_permutation_mnk(
    ...
    perm_m = max(tile_shape_mnk[0], 128)
    perm_n_shape = (8, 2, 2)
...
def sm120_make_smem_layout_sfa(
    ...
    *,
    ...
    blk_mn = 128 if tile_shape_mnk[0] % 128 == 0 else 96  # type: ignore[operator]
    blk_sf = 4
    blk_elems = blk_mn * blk_sf
    mma_nsf = tiled_mma.shape_mnk[2] // sf_vec_size

    mn_basic_block_shape = (32, blk_mn // 32)
    mn_basic_block_stride = ((blk_mn // 32) * blk_sf, blk_sf)
    k_basic_block_shape = (sf_vec_size, mma_nsf)
    k_basic_block_stride = (0, 1)
...
def tile_atom_to_shape_SF_blk(shape, sf_vec_size, blk_mn=128):
    blk_sub = blk_mn // 32
    chunk = cute.make_layout(
        ((32, blk_sub), (sf_vec_size, 4)),
        stride=((blk_sub * 4, 4), (0, 1)),
    )
...
class BlockscaledGemmKernel:
    ...
    def __init__(
        ...
        # MMA atom m16n8k64 tiled as 6 warps in M x 2 in N; _setup_attributes
        # reads both. num_m_tiles / num_n_tiles / num_k_blocks are floor
        # divisions there, so a tile that is not a whole number of warp groups
        # would silently drop the remainder -- check it here instead.
        self.warp_shape = (6, 2, 1)
        self.mma_shape_mnk = (16, 8, 64)
        group_m = self.mma_shape_mnk[0] * self.warp_shape[0]
        group_n = self.mma_shape_mnk[1] * self.warp_shape[1]
        assert tile_m % group_m == 0, (
            f"tile_m ({tile_m}) must be a multiple of group_m ({group_m} = "
            f"{self.mma_shape_mnk[0]} * warps_m={self.warp_shape[0]}) for the "
            f"12-MMA-warp layout"
        )
        assert tile_n % group_n == 0, (
            f"tile_n ({tile_n}) must be a multiple of group_n ({group_n} = "
            f"{self.mma_shape_mnk[1]} * warps_n={self.warp_shape[1]})"
        )
        assert tile_k % self.mma_shape_mnk[2] == 0, (
            f"tile_k ({tile_k}) must be a multiple of the MMA instruction k "
            f"({self.mma_shape_mnk[2]})"
        )
        ...
        if load_register_requirement is None:
            load_register_requirement = 32
        assert (
            24 <= load_register_requirement <= 256
            and load_register_requirement % 8 == 0
        ), "load_register_requirement must be a multiple of 8 in [24, 256]"
        self.load_register_requirement = load_register_requirement
    ...
    def _setup_attributes(self):
        ...
        permutation_mnk = get_permutation_mnk(
            self.tile_shape_mnk, self.sf_vec_size, False
        )
        self.tiled_mma = cute.make_tiled_mma(
            mma_op,
            cute.make_layout(self.warp_shape),
            permutation_mnk=permutation_mnk,
        )

        warp_shape = self.warp_shape

        mma_m, mma_n, mma_k = self.mma_shape_mnk
        ...
        group_m = mma_m * warp_shape[0]
        assert self.epi_tile[0] % group_m == 0, (
            f"epi_tile_m ({self.epi_tile[0]}) must be a multiple of group_m "
            f"({group_m} = 16 * warps_m={warp_shape[0]}); valid values are multiples "
            f"of {group_m} that divide tile_m, e.g. --epi_tile_m {group_m}"
        )

        self.cta_layout_mnk = cute.make_layout(self.cluster_shape_mnk)

        self.clc_cluster_layout_vmnk = cute.make_layout((1, 1, 1, 1))
    ...
    def __call__(
        ...
        self.sfa_layout = tile_atom_to_shape_SF_blk(
            a.shape, self.sf_vec_size, self.sfa_sf_blk_mn
        )
        ...
        class SharedStorage:
            ...
            clc_response: cute.struct.MemRange[cutlass.Int32, 4]  # 16-byte try_cancel response
    ...
    def kernel(
        ...
        num_clc_consumer_threads = self.num_threads_per_warp * (
            1 + (1 + 1 + self.num_mma_warps)
        )
        ...
        clc_pipeline = pipeline.PipelineClcFetchAsync.create(
            ...
            defer_sync=False,  # self-init; a deferred init is never published on a 1-CTA cluster
        )
        ...
        def _regroup_sf(tCrSF_full, mma_mn_size):
            rank = cute.rank(tCrSF_full)
            prod = 1
            n_mn = 0
            for i in cutlass.range_constexpr(1, rank):
                prod *= cute.size(tCrSF_full, mode=[i])
                n_mn += 1
                if cutlass.const_expr(prod == mma_mn_size):
                    break
            tCrSF_full = cute.group_modes(tCrSF_full, 1, 1 + n_mn)
            tCrSF_full = cute.group_modes(tCrSF_full, 2, cute.rank(tCrSF_full))
            return tCrSF_full

        tCrSFA_full = _regroup_sf(tCrSFA_full, cute.size(tCrA, mode=[1]))
        tCrSFB_full = _regroup_sf(tCrSFB_full, cute.size(tCrB, mode=[1]))
    ...
    def _compute_grid(
        ...
        tile_sched_params = utils.ClcDynamicPersistentTileSchedulerParams(
            num_ctas_mnl, cluster_shape_mnl, eff_swizzle, raster_along_m
        )
        print(
            f"CLC tile scheduler: swizzle_size={eff_swizzle}, "
    ...
    def can_implement(
        ...
        if cluster_shape_mn != (1, 1):
            return False
        if tuple(tile_shape_mnk) not in ((128, 128, 128), (128, 128, 256), (256, 128, 128), (192, 128, 128)):
            return False
...
def _reorder_sf_mkl_to_tile_atom_torch(sf_mkl: torch.Tensor, sf_vec_size: int = 16, blk_mn: int = 128):
    mn, sf_k, l = sf_mkl.shape

    blk_sub = blk_mn // 32
    pad_mn = ((mn + blk_mn - 1) // blk_mn) * blk_mn

    device = sf_mkl.device
    out = torch.zeros((pad_mn * sf_k * l,), dtype=sf_mkl.dtype, device=device)
    ...
    atom_m0 = mn_idx % 32
    atom_m1 = (mn_idx // 32) % blk_sub
    rest_m = mn_idx // blk_mn

    atom_k1 = sfk_idx % 4
    rest_k = sfk_idx // 4

    rest_m_extent = pad_mn // blk_mn
    rest_k_extent = sf_k // 4
    atom_cosize = 32 * blk_sub * 4
...
def create_and_reorder_scale_factor_tensor(
    l, mn, k, sf_vec_size, sf_dtype, torch_tensor, blk_mn=128
):
    ...
    return _reorder_sf_mkl_to_tile_atom_torch(torch_tensor, sf_vec_size, blk_mn)
...
class Sm120Fp4GemmLauncher:
    ...
    def __init__(
        ...
        self.kernel = BlockscaledGemmKernel(
            ...
            load_register_requirement=load_register_requirement,
        )
...
def run_scaled_mm(
    ...
    **kwargs,
    ...
    sfa_reordered = create_and_reorder_scale_factor_tensor(
        l, m, k, sf_vec_size, sf_dtype, sfa, (128 if tile_shape_mnk[0] % 128 == 0 else 96)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=parse_comma_separated_ints,
        choices=[
            # tile_m must be a multiple of group_m = 96 (6 M-warps x 16), so the
            # 128/256 shapes carried over from the 8-warp versions cannot run here.
            (192, 128, 128),
        ],
        default=(192, 128, 128),
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="DMA/store-warp register cap (multiple of 8 in [24,256]); default 32",
    )
    parser.add_argument(
        "--sf_tma_internal_type",
        choices=["none", "int16"],
    ...
    exec_time = run(
        ...
        ab_stage=args.ab_stage,
        epi_stage=args.epi_stage,
        raster_along_m=args.raster_along_m,
        sf_tma_internal_type=args.sf_tma_internal_type,
        load_register_requirement=args.load_register_requirement,
    )
    m, n, k, l = args.mnkl
    flops = 2.0 * m * n * k * l
