...
class BlockscaledGemmKernel:
    ...
    def __init__(
        ...
        swizzle_size: int = 1,
        epi_tile_m: Optional[int] = None,
        epi_tile_n: Optional[int] = None,
        ab_stage: Optional[int] = None,
        epi_stage: Optional[int] = None,
    ):
    ...
    def _setup_attributes(self):
        ...
        computed_ab_stage, self.epi_stage, epi_stage_max = self._compute_stages(
            self.tile_shape_mnk,
            self.a_dtype,
            self.b_dtype,
            self.sf_dtype,
    ...
    def kernel(
        ...
        if warp_idx < self.num_mma_warps:
            ...
            tCsSFA_copy_view_full = thr_copy_SFA.partition_S(sSFA)
            tCrSFA_copy_view_full = thr_copy_SFA.retile(tCrSFA_full)
            tCsSFB_copy_view_full = thr_copy_SFB.partition_S(sSFB)
            tCrSFB_copy_view_full = thr_copy_SFB.retile(tCrSFB_full)

            epi_rest_m_static = self.tile_shape_mnk[0] // self.epi_tile[0]
            epi_rest_n_static = self.tile_shape_mnk[1] // self.epi_tile[1]
            has_multi_epi_store = cutlass.const_expr(
                not (
                    self.epi_stage == 1
                    and epi_rest_m_static == 1
                    and epi_rest_n_static == 1
                )
            )
            ...
            while work_tile.is_valid_tile:
                ...
                if cutlass.const_expr(self.sfa_tiles_per_block > 1):
                    ...
                    tCsSFA_tile_copy_view = thr_copy_SFA.partition_S(sSFA_tile)
                ...
                if cutlass.const_expr(self.sfb_tiles_per_block > 1):
                    ...
                    tCsSFB_tile_copy_view = thr_copy_SFB.partition_S(sSFB_tile)
            ...
            # All work tiles done: drain any in-flight TMA stores exactly once.
            if has_multi_epi_store:
    ...
    def _compute_stages(
        ...
        computed_epi_stage = min(epi_stage_max, 4)
    ...
    def _compute_grid(
        ...
        try:
            divisor_tile_cnt = int(num_ctas_mnl[1])
        ...
        try:
            ...
            print(f"Tile scheduler: swizzle_size={eff_swizzle}, raster_along_m=True")
        except TypeError:
            tile_sched_params = utils.PersistentTileSchedulerParams(
    ...
    def can_implement(
        ...
        tile_shape_mnk: Tuple[int, int, int],
        cluster_shape_mn: Tuple[int, int],
        epi_tile_m: Optional[int],
        epi_tile_n: Optional[int],
        ab_stage: Optional[int],
        epi_stage: Optional[int],
        m: int,
        n: int,
        k: int,
        l: int,
        ...
        if cluster_shape_mn != (1, 1):
            return False
        # Tile M must be divisible by 128
        if tile_shape_mnk[0] % 64 != 0 or tile_shape_mnk[1] % 64 != 0:
            return False
        ...
        tile_k = tile_shape_mnk[2]
        if k % tile_k != 0:
            return False

        if epi_tile_m in (0, None):
            epi_m = tile_shape_mnk[0]
        else:
            epi_m = min(epi_tile_m, tile_shape_mnk[0])
        if epi_tile_n in (0, None):
            epi_n = tile_shape_mnk[1]
        else:
            epi_n = min(epi_tile_n, tile_shape_mnk[1])
        if tile_shape_mnk[0] % epi_m != 0 or epi_m % 16 != 0:
            return False
        if tile_shape_mnk[1] % epi_n != 0 or epi_n % 16 != 0:
            return False

        sfa_tile_m = max(128, ((tile_shape_mnk[0] + 127) // 128) * 128)
        sfb_tile_n = max(128, ((tile_shape_mnk[1] + 127) // 128) * 128)
        ab_bytes_per_stage = (tile_shape_mnk[0] * tile_k + tile_shape_mnk[1] * tile_k) // 2
        sf_bytes_per_stage = (sfa_tile_m // 128) * 4 * 128 + (sfb_tile_n // 128) * 4 * 128
        epi_stage_max = (tile_shape_mnk[0] // epi_m) * (tile_shape_mnk[1] // epi_n)
        computed_epi_stage = min(epi_stage_max, 4)
        if epi_stage is None:
            effective_epi_stage = computed_epi_stage
        else:
            if not (1 <= epi_stage <= epi_stage_max):
                return False
            effective_epi_stage = epi_stage
        epi_bytes = epi_m * epi_n * (c_dtype.width // 8) * effective_epi_stage
        mbar_bytes = 1024
        ...
        computed_ab_stage = max(1, min(computed_ab_stage, 4))
        if ab_stage is not None and not (1 <= ab_stage <= computed_ab_stage):
            return False
        if computed_ab_stage < 1:
            return False
        return True
...
def _reorder_sf_mkl_to_tile_atom_torch(sf_mkl: torch.Tensor, sf_vec_size: int = 16):
...
def create_and_reorder_scale_factor_tensor(
    ...
    return _reorder_sf_mkl_to_tile_atom_torch(torch_tensor, sf_vec_size)
...
def run_scaled_mm(
    ...
    **kwargs,
    ...
    print(f"Tile Shape (M, N, K): {tile_shape_mnk}, Cluster Shape (M, N): {cluster_shape_mn}")
    ...
    print(f"Epilogue tile: {epi_tile_m or tile_shape_mnk[0]}x{epi_tile_n or tile_shape_mnk[1]}")
    print(f"AB stage override: {ab_stage}")
    print(f"Epilogue stage override: {epi_stage}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Persistent tile scheduler swizzle size. Use 1 to preserve the original raster order.",
    )
    parser.add_argument(
        "--epi_tile_m",
        type=int,
        default=0,
        help="Epilogue tile M. Use 0 to default to the CTA tile M.",
    )
    parser.add_argument(
        "--epi_tile_n",
        type=int,
        default=0,
        help="Epilogue tile N. Use 0 to default to the CTA tile N.",
    )
    parser.add_argument(
        "--ab_stage",
        type=int,
        default=0,
        help="Mainloop pipeline stages. Use 0 to use the computed default.",
    )
    parser.add_argument(
        "--epi_stage",
        type=int,
        default=0,
        help="Epilogue TMA-store pipeline stages. Use 0 to use the computed default.",
    )
    ...
    if args.swizzle_size < 1:
        parser.error("--swizzle_size must be >= 1")

    if args.epi_tile_m < 0 or args.epi_tile_n < 0:
        parser.error("--epi_tile_m and --epi_tile_n must be >= 0")
    if args.epi_tile_m not in (0,) and (
        args.tile_shape_mnk[0] % args.epi_tile_m != 0 or args.epi_tile_m % 16 != 0
    ):
        parser.error("--epi_tile_m must divide tile M and be a multiple of 16")
    if args.epi_tile_n not in (0,) and (
        args.tile_shape_mnk[1] % args.epi_tile_n != 0 or args.epi_tile_n % 16 != 0
    ):
        parser.error("--epi_tile_n must divide tile N and be a multiple of 16")
    if args.ab_stage < 0 or args.epi_stage < 0:
        parser.error("--ab_stage and --epi_stage must be >= 0")

    epi_tile_m = None if args.epi_tile_m == 0 else args.epi_tile_m
    epi_tile_n = None if args.epi_tile_n == 0 else args.epi_tile_n
