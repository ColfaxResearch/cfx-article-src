...
template<typename T> void cute_transpose_unpack(torch::Tensor input, torch::Tensor output, Version ver) {
...
// This function is bound to "cute_transpose.transpose". 
torch::Tensor cute_transpose(torch::Tensor input,
  ...
  if(!(_input.device().is_cuda() && _output.device().is_cuda()))
    throw std::invalid_argument("cute_transpose only supports GPU device. Use .to(device=torch.device('cuda'))");

  // Select the CUTLASS precision type to use based on Torch input data type.
  if(_input.dtype() == torch::kFloat16)
    cute_transpose_unpack<cutlass::half_t>(_input, _output, ver);
  else if(_input.dtype() == torch::kFloat32)
    cute_transpose_unpack<float>(_input, _output, ver);
  else
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  py::enum_<Version>(m, "version")
      ...
      .value("tma", tma)
      .export_values();
  m.def("transpose", py::overload_cast<torch::Tensor,c10::optional<torch::Tensor>,Version>(&cute_transpose), py::arg("input"), py::arg("output") = py::none(), py::arg("version")=swizzle);
